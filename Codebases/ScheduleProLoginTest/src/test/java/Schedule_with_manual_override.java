...
public class Schedule_with_manual_override {
  public static void main(String[] args){
      // Todo : Setting up ExtentReport package to give us test results.
      ExtentReports extent = new ExtentReports();
      SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss");
      String timestamp = sdf.format(new Date());
      String filename = "target/Reports/SchedulePO_with_manual_override-" + timestamp + ".html";
...
//            Clearing existing input
    ...
    public static void SchedulePO(WebDriver driver, Properties prop, ExtentReports extent){
        try{
            ...
            String targeted_PO_number = prop.getProperty("manual_target_po_number");
            ...
            if(click_on_text.isDisplayed()){
                ...
                ExtentTest test_Schedule_by_Recommendation = extent.createTest("Schedule this PO by the Manual Override",test3_desc);
                ...
                if(date_time_select_text.isDisplayed()){
                    test_Schedule_by_Recommendation.pass("We've Chosen manual override and reached at the date and time picker section");
                }
                else{
                ...
                // Select Date
                String target_date = prop.getProperty("manual_date");

                String just_date = target_date.split("-")[0];

                String date_select_info = "Selecting Appointment Date as : %s th of March";

                String selected_date_info = String.format(date_select_info, just_date);

                test_Select_Date_and_Time.info(selected_date_info);

                String xpath_date_selector = "/html/body/div/div[2]/div/div[2]/div/div/div[2]/div/div[2]/div/div[1]/div[1]/div/div[1]/div[3]/button[%s]";
                String dynamicXpath2 = String.format(xpath_date_selector, just_date);

                driver.findElement(By.xpath(dynamicXpath2)).click(); // In this step we are selecting the date.

                test_Select_Date_and_Time.info("Waiting 4 seconds for the available time slots to appear.");
                Thread.sleep(4600);

                String target_time = prop.getProperty("manual_time");
                // Select Time

                String time_selection_info = "Selecting time as : %s ";
                String selected_time_slot_info = String.format(time_selection_info, target_time);

                test_Select_Date_and_Time.info(selected_time_slot_info);

                String time_selection_xpath = "//div[@class=\"time-slot-ss\" and text()=\"" + target_time + "\"]";

                driver.findElement(By.xpath(time_selection_xpath)).click();
...
//                After selecting the carrier we can now finally click on the submit button
                ...
                test_schedule_PO_final.info("Successfully Scheduled an Appointment for PO number : " + targeted_PO_number + " with date : " + ap_date + " and Time : " + ap_time);

                // Here we'll verify that I date and time we've selected earlier where shown in the scheduled appointment.
                test_schedule_PO_final.info("Finally we compare the date and time we've selected with the scheduled appointment.");

                just_date = just_date + "th March 2024";


                boolean matching_date = ap_date.equals(just_date);
                boolean matching_time = ap_time.equals(target_time);

                System.out.println("AP Date : " +  ap_date);
                System.out.println("AP time : " +  ap_time);
                System.out.println("Target date : " + just_date);
                System.out.println("Target Time : " + target_time);
                System.out.println("Date matching : " + matching_date);
                System.out.println("Time matching : " + matching_time);

                String compare_dates = "Targeted date : " + just_date + " & Scheduled Date : " + ap_date ;
                test_schedule_PO_final.info(compare_dates);

                String compare_time = "Targeted time : " + target_time + " & Scheduled Time : " + ap_time;
                test_schedule_PO_final.info(compare_time);

                if(matching_time && matching_date){
                    test_schedule_PO_final.pass("Validation successful. We've scheduled the appointment with correct date and time.");
                }
                else{
                    test_schedule_PO_final.fail("Validation failed. We've failed to schedule the appointment with correct date and time.");
                }
